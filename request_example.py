...
request_output_dir = os.path.join(os.path.dirname(__file__), "request_example_results")
...
def dump_json(filename: str, json_dict: dict[str, Any]) -> None:
    file_path = os.path.join(request_output_dir, f"{filename}.json")
    with open(file_path, "w", encoding="utf8") as output_json:
...
def term_entries() -> None:
    ...
    response = requests.post(
        request_url + "/termEntries", json=params, timeout=request_timeout
    )
    print(response)
...
def kanji_entries() -> None:
    ...
    response = requests.post(
        request_url + "/kanjiEntries", json=params, timeout=request_timeout
    )
    print(response)
    print(elide(response.text))
...
def anki_fields_term() -> None:
    ...
    response = requests.post(
        request_url + "/ankiFields", json=params, timeout=request_timeout
    )
    print(response)
    print(elide(response.text))
...
def tokenize() -> None:
    ...
    response = requests.post(
        request_url + "/tokenize", json=params, timeout=request_timeout
    )
    print(response)
    print(elide(response.text))
